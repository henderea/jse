...
export class VarList {
  ...
  get localVars() {
    this.loadLocalVarsIfNeeded();
    return this._localVars;
  }

  set localVars(value) {
    this._localVars = value;
  }

  sortVars() {
    if(this.saveLocal) {
      this.localVars = sortObjectByKeys(this.localVars);
      this.saveLocalVars();
    } else {
  ...
  loadLocalVarsIfNeeded() {
    if(this._lastWd != state.wd) { this.loadLocalVars(); }
    this._lastWd = state.wd;
  }

  loadLocalVars() {
    this.localVars = readJsonOrDefault(path.join(state.wd, 'vars.jse.json'), {});
  }

  saveLocalVars() {
    if(!this._noSave) {
      fs.writeFileSync(path.join(state.wd, 'vars.jse.json'), JSON.stringify(this.localVars, null, 2));
    }
  }
  ...
  get(varName) {
    this.loadLocalVarsIfNeeded();
    return (this.hasLocal(varName) && this._searchLocal) ? this.localVars[varName] : this._vars[varName];
  }
  ...
  hasLocal(varName) {
    this.loadLocalVarsIfNeeded();
    return Object.prototype.hasOwnProperty.call(this.localVars, varName);
  }
  ...
  get keys() {
    this.loadLocalVarsIfNeeded();
    return _uniq(_concat([], _keys(this.localVars), _keys(this._vars)));
  }
  ...
  get localKeys() {
    this.loadLocalVarsIfNeeded();
    return _uniq(_keys(this.localVars));
  }
